...
const root = ReactDOM.createRoot(document.getElementById('seating-plan'))
root.render(<SeatingPlan defaultRowCount={5} initialAssignment={initialAssignment} initialHiddenSeats={initialHiddenSeats}></SeatingPlan>)

const itemCounter = (value, index) => {
    return value.filter((x) => x == index).length
}

function SeatingPlan({defaultRowCount, initialAssignment, initialHiddenSeats}) {
...
function Order({order_id, order, seats_assigned}) {
	...
	return (
		<div className={order.seats === seats_assigned ? "order seated" : "order"} data-order-id={order_id} draggable={true} onDragStart={(e) => handleDragStart(e, `${order_id}`)}>
			<h2 className={"num"}><span>{order.seats - seats_assigned}</span></h2>
			<h3 className={"name"}>{order.name}</h3>
			<p className={"seats"}><span className={"bold"}></span>Adult: {order.adults}, Child: {order.children}</p>
		</div>
	)
}
